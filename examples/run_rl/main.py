...
from hold_policies.environments.utils import get_goal_example_environment_from_variant
from hold_policies.environments.adapters import robodesk_adapter
from hold_policies.algorithms.utils import get_algorithm_from_variant
from hold_policies.policies.utils import get_policy_from_variant, get_policy
from hold_policies.replay_pools.utils import get_replay_pool_from_variant
from hold_policies.samplers.utils import get_sampler_from_variant
from hold_policies import samplers
from hold_policies.value_functions.utils import get_Q_function_from_variant
from hold_policies.models.utils import get_inverse_model_from_variant
from hold_policies.misc.generate_goal_examples import get_goal_example_from_variant
from hold_policies.preprocessors.utils import get_preprocessor_from_params


from hold_policies.misc.utils import set_seed, initialize_tf_variables
...
class ExperimentRunnerRL(ExperimentRunner):
    ...
    def _build(self):
        ...
        # if variant['sampler_params']['type'] == 'DistanceModelSampler':
        #     import predict_with_vivit
        #     predict_with_vivit.load_and_predict()
        #     del predict_with_vivit

        training_environment = self.training_environment = (
            get_goal_example_environment_from_variant(variant))
        evaluation_environment = self.evaluation_environment = (
            get_goal_example_environment_from_variant(variant, evaluation=True))
    ...
    def picklables(self):
        ...
        if not isinstance(self.sampler, samplers.DistanceModelSampler):
            picklables['sampler'] = self.sampler
        if not isinstance(self.training_environment._env.env,
                          robodesk_adapter.RoboDeskAdapter):
            picklables['training_environment'] = self.training_environment
        if not isinstance(self.evaluation_environment._env.env,
                          robodesk_adapter.RoboDeskAdapter):
            picklables['evaluation_environment'] = self.evaluation_environment
...
def main(argv=None):
    ...
    run_example_debug('examples.run_rl', argv)
