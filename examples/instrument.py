...
from hold_policies.misc.utils import datetimestamp, PROJECT_PATH
...
def generate_experiment_kwargs(variant_spec, command_line_args):
    # TODO: Allow local dir to be modified through cli args
    local_dir = os.path.join(
        os.environ['HOLD_TOP_DIR'],
        'hold_policies/ray_results',
    ...
    # Add path info to experiment config in order to save trajectories from
    # RLAlgorithm to Ray's checkpoint directory.
    variant_spec['experiment_id'] = experiment_id
    variant_spec['local_dir'] = local_dir
    ...
    if command_line_args.eval_path_save_frequency is not None:
        assert 'algorithm_params' in variant_spec
        variant_spec['algorithm_params']['kwargs'][
            'eval_path_save_frequency'] = (
                command_line_args.eval_path_save_frequency)
...
def run_example_local(example_module_name, example_argv, local_mode=False):
    ...
    # module name, e.g. examples.run_rl
    # import pdb; pdb.set_trace()
    example_module = importlib.import_module(example_module_name)

    # example_argv: command line args
    # example_args: module's default values overwritten by command line args
    example_args = example_module.get_parser().parse_args(example_argv)
