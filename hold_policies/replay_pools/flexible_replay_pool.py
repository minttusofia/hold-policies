...
class FlexibleReplayPool(ReplayPool):
    def __init__(self, max_size, fields_attrs, compress_observations=False):
        super(FlexibleReplayPool, self).__init__()

        max_size = int(max_size)
        self._max_size = max_size
        self._compress_observations = compress_observations

        self.fields = {}
        self.fields_attrs = {}

        self._fields_to_compress = []
        if self._compress_observations:
            self._fields_to_compress = ['observations', 'next_observations']
        for field_name in self._fields_to_compress:
            fields_attrs[field_name]['shape'] = ()
            fields_attrs[field_name]['dtype'] = np.object
    ...
    def add_samples(self, samples):
        ...
        for field_name in self.field_names:
            ...
            if field_name in self._fields_to_compress:
                # Assumes image shape does not vary over the batch.
                values = np.reshape(values, (-1,) + samples['infos'][0]['image_shape'])
                values = (values * 255).astype(np.uint8)
                values = np.array([compress_image(img) for img in values])
    ...
    def batch_by_indices(self, indices, field_name_filter=None):
        ...
        for field_name in self._fields_to_compress:
            imgs = np.array(
                [decompress_image(img) for img in batch[field_name]])
            imgs = imgs / 255.
            batch[field_name] = np.reshape(
                imgs, (batch[field_name].shape[0], -1))
        return batch
